...
exports.pageData = function(req, res) {
  ...
  Git.pull(app.locals.remote, app.locals.branch, function(err) {
    if (err) return error500(req, res, err);
    Git.readFile(pageName + ".md", "HEAD", function(err, content) {
      if (err) res.redirect('/pages/new/' + pageName);
...
exports.pageEdit = function(req, res) {
  ...
  return res.redirect('/livepreview/index.html?page='+pageName);
...
exports.ticketUpdate = function (ticket, title, content, comment) { 
  var edit = '[[url:'+app.locals.baseUrl+'/livepreview/index.html?page='+ticket+'-'+title+'|Edit]]\n'; 
  var content = JSON.stringify({ticket:{summary:title, description:edit+'<pre><code>'+content+'</code></pre>'}});
  console.log('EDIT: '+edit); 
  var opts  = {
     hostname:app.locals.ticketHost, 
     port:443, 
     method:'put',
     path:app.locals.ticketPath+ticket,
     auth:app.locals.ticketAuth,
     headers:{Accept:'application/json', 'content-length':content.length, 'Content-type':'application/json'},
  };
  console.log('updating '+opts.hostname+opts.path);
  var req = Https.request(opts, function(res) { 
  //   res.on('data', function(chunk) { console.log('ASSEMBLA RESPONSE: '+chunk) }) 
  });
  req.end(content);
...
*/
  ...
  if (req.body.pageTitle) content = "# " + req.body.pageTitle + "\n" + req.body.content.replace(/\r\n/gm, "\n");
  ...
  if (match)  exports.ticketUpdate(match[1], match[2], content, message);
